<template>
  <v-data-table item-key="name" class="elevation-1" :loading="isLoading" :loading-text="$t('misc.LoadText')"
    :headers="headers" :items="matches" :options.sync="options" :server-items-length="totalMatches" ref="MatchesTable">
...
export default {
  ...
  methods: {
    /**
     * @param {Array<Match>} resultArray - Array of match objects
     */
    async pushMatchData(resultArray) {
      ...
      resultArray = resultArray.sort((a, b) => {
        console.log(a.cancelled, b.match_status)
        const aLive = a.start_time < new Date() && !a.end_time && !a.cancelled;
        const bLive = b.start_time < new Date() && !b.end_time && !b.cancelled;
